...
//        List<String> directionList = new ArrayList<>();
    ...
    public void test08() {
        ...
        //     * D             500
        //     * M             1000
//        int[] values = new int[]{1, 4, 5, 9, 1, 4, 5, 9, 1, 4, 5, 9, 1};
        int[] values = new int[]{1, 4, 5, 9, 1, 4, 5, 9, 1, 4, 5, 9, 1};
...
//        String[] symbols = new String[]{"I", "IV", "V", "IX", "X", "XL", "L", "XC", "C", "CD", "D", "CM", "M"};
        ...
        for (int i = 0; i < length; i++) {
            temp = s.charAt(i) - 48;
            if (i == 0 && length == 4) {
                quotient = temp / values[12];
                if (quotient > 0) {
                    for (int m = quotient; m > 0; m--) {
                        result.append(symbols[12]);
                    }
                }
            } else {
                for (int j = 4 * (length - 1 - i) + 3; j >= 4 * (length - 1 - i); j--) {
                    quotient = temp / values[j];
                    if (quotient > 0) {
                        for (int m = quotient; m > 0; m--) {
     ...
     * Now you want to find out who the celebrity is or verify that there is not one. The only thing you are allowed to do is to ask questions like: “Hi, A.
     * Do you know B?” to get information of whether A knows B. You need to find out the celebrity (or verify there is not one) by asking as few questions as possible (in the asymptotic sense).
        ...
        for (int i = 0; i < n; i++) {
            if (i != cel) {
                if (!knows(i, cel) || knows(cel, i)) {
                    System.out.println("celebrity is: " + -1);
                    return;
                }
            }
        }
    ...
    private boolean knows(int m, int n) {
        return true;
    }
    ...
    public void test13() {
        ...
        int len = gas.length;
        int[] needGas = new int[len];
        for (int i = 0; i < len; i++) {
            needGas[i] = gas[i] - cost[i];
        }

        int startingIndex = len - 1;
        int maxGas = needGas[len - 1];
        int sum = 0;
        ...
        System.out.println("starting station: " + (sum >= 0 ? startingIndex : -1));
    }
    ...
    private List<String> generateParentheses(int n) {
        ...
        recGenerateParentheses(resultList, "", 0, 0, n);
        return resultList;
    }

    private void recGenerateParentheses(List<String> resultList, String s, int left, int right, int n) {
        if (left == n && right == n) {
            resultList.add(s);
        }else {
            if (left < n) {
                recGenerateParentheses(resultList, s + "(", left + 1, right, n);
            }
            if (right < left) {
                recGenerateParentheses(resultList, s + ")", left, right + 1, n);
            }
        }
    }
}
